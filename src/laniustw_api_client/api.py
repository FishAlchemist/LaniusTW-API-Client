...
from laniustw_api_client import PROJECT__API_URL
import pathlib


@validate_call
def test(identifier: uuid.UUID | None) -> httpx.Response:
    api_url = f"{PROJECT__API_URL}/api/test"
...
def speech_recogn(speech_file: pathlib.Path) -> httpx.Response:
    api_url = f"{PROJECT__API_URL}/api/speech_recognition"
...
def image_recognition(file: pathlib.Path) -> httpx.Response:
    api_url = f"{PROJECT__API_URL}/api/Image_recognition"
...
def chat_RAG(question: str, identifier: uuid.UUID | None) -> httpx.Response:
    api_url = f"{PROJECT__API_URL}/api/chat/RAG"
    params = {
        "question": question,
...
def chat_Recommendation(
    ...
    api_url = f"{PROJECT__API_URL}/api/chat/Recommendation"
    params = {
        "question": question,
        "identifier": identifier,
...
def chat_Experience(question: str, identifier: uuid.UUID | None):
    api_url = f"{PROJECT__API_URL}/api/chat/Experience"
    params = {
        "question": question,
        "identifier": identifier,
    }
